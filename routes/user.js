...
const redirectUrl = process.env.clientOrigin;
        ...
        bcrypt.hash(password, 10, (err, hash) => {
          if (err) {
            ...
            newUser
              .save()
              .then(() => {
                sendEmail(newUser.email, templates.confirm(newUser._id)).then(
                  () =>
                    res.status(200).json({
                      msg1: "User Added!",
                      msg2: msgs.confirm,
    ...
    .then((user) => {
      ...
      // confirmed, then confirm the user's email
        ...
        user
          .save()
          .then(() =>
            res.process.env.clientOrigin(process.env.clientOrigin)
          )
          .catch((err) =>
...
router.put("/addFriend/:id", checkAuth, (req, res, next) => {
  ...
  // list. NOTE: The 'userId' should always be sent first in the URL followed
  // by the friends 'userId', and they should be separated by a '&'
